...
window.cloverStore = {
  ...
  setDashPanels(arr) { state.settings.dashPanels = (Array.isArray(arr) && arr.length) ? arr.map(p => ({ k: p.k, c: p.c ? 1 : 0, w: (p.w === 1 || p.w === 2 || p.w === 3) ? p.w : 0, h: p.h ? 1 : 0, off: p.off ? 1 : 0 })) : null; scheduleSave(); notify(); },
